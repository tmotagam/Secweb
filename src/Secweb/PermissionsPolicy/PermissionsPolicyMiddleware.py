...
class PermissionsPolicy(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if self.Option == {}:
            ...
            warnings.warn("Permission-Policy header is still under working draft, browsers might give warnings and errors", SyntaxWarning, 2)
            ...
            if keys[i] in self.Policy:
                ...
                for j in range(len(values)):
                    if values[j] != 'self' and values[j] != '*':
                        vch = values[j]
                        if vch[0] != '"':
                            raise SyntaxError(f'Invalid allowlist item({ vch }) for feature { keys[i] }. Allowlist item must be *, self or quoted url "https://example.com" ')
